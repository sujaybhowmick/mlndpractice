...
def avg_medal_count():
    ...
    # avg_medals = df[['gold', 'silver', 'bronze']].apply(numpy.mean)
    medal_points = df[['gold', 'silver', 'bronze']]

    total_medal_points = numpy.dot(medal_points, [4, 2, 1])

    # print avg_bronze_silver
    return DataFrame({'country_name': Series(countries), 'points': Series(total_medal_points)})
